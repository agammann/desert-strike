...
html = html.replace('<script src="src/campaigns.js"></script><script src="src/simulation.js"></script><script src="src/game.js"></script>', () => `<script>window.GULF_ASSETS=${JSON.stringify({ terrain: `data:image/png;base64,${terrain}`, sprites: `data:image/png;base64,${sprites}` })};</script><script>${campaigns}</script><script>${sim}</script><script>${game}</script>`);
